...
recordings - Freesound CC0 / CC-BY only, never non-commercial - only if it stops feeling right in play. Menus done (title / pause / settings / game over). Gregg is painting graffiti STRIKE / SPARE / MISS / GUTTER call-outs -> `games/bowling/art/callouts/` (spec in its README). Local multiplayer done 2026-09-19 (1-4 bowlers, hot seat, names + a ball each, standings). Open: online play over ManorNet, a lighter web GLB per alley (web pck is ~41 MB). Maybe later: Gregg may render a hi-res ghost-bride panel from 3D characters (art-stream idea) to replace the code-drawn `BWL_BridePanel.png` (1024+ square, arch-cut, dark background with glowing art; drop in and rebuild with `--crypt`) — see `games/bowling/README.md`, PRD in `docs/PRD_PhantomBowling.md` |
